...
> - **Note (learning vs. real implementation):** the 0.1.5 learning notebook (`phase0_learning.ipynb`) intentionally implements the *paper-literal* unclamped ACI (`α_t` allowed below 0/above 1, infinite/empty sets) to make the self-correction mechanism visible. For the actual flagship method (Phase 2 / 2.2a), don't re-derive ACI from the paper — implement against **herbps10/AdaptiveConformal** directly (clamped `α_t`/`θ_t` to a safe range, e.g. `[0.01, 0.5]` for target `α=0.1`), and likewise use **ryantibs/conformal**'s `weighted.quantile` convention as-is rather than a fresh derivation. Track how often the update hits the clamp — that's the abstain/route trigger for OOD routing, same role the unclamped demo's `is_inf` counter plays.
