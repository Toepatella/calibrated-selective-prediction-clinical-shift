...
- `w_lab(y) := p_T(y) / p_S(y)` — **label / prevalence ratio**; equals the full `w(x,y)` *iff* the shift is pure label. Estimated by **BBSE** (Lipton, Wang & Smola, ICML 2018) from the source confusion matrix and target predictions:
  ...
  - **Why MLLS is the recommended estimator under the *factorizable* premise (not merely a preferred baseline).** BBSE's consistency requires the anti-causal invariance `P_T(ŷ=i∣y=k) = P_S(ŷ=i∣y=k)` (an invariant confusion `C_S`). The factorizable / invariant-density-ratio condition this pipeline targets — `p_T(x∣y)/p_S(x∣y)` class-free on overlapping support (Tasche 2022, arXiv:2207.14514, Thm 4/Cor 4 at ρ=1) — does **not** imply that invariance: a *classifier-visible* class-free covariate tilt (one that moves the `ŷ∣y` histogram) is factorizable yet violates the BBSE assumption, and BBSE then carries an `O(1)` bias that does **not** shrink in `n` (a class-free `exp(β·tanh(x₁))` tilt gives population BBSE `≈(1.06, 0.94)` against a truth of `(1.8, 0.2)`, a bias `≈0.74`, while `P_T(ŷ∣y)` moves `≈0.27` from `P_S(ŷ∣y)`). MLLS+BCTS, by contrast, is consistent under the factorizable premise given only calibration (Garg et al. 2020). So under this pipeline's premise the two estimators are **not** interchangeable: MLLS+BCTS is the recommended `ŵ_lab`, BBSE is kept as a baseline and as the anti-causal consistency diagnostic of §3 (its `q̂_T` vs `Ĉ_S p̂_T` check is precisely a *test* of the invariance BBSE needs).
...
| **Label / prevalence shift** `p(y)` moves, `p(x∣y)` fixed | Covariate-only reweighting cannot capture a *label-marginal* change; naïvely multiplying `w_cov·w_lab` **double-counts** | **MLLS+BCTS `ŵ_lab`** (recommended under the factorizable premise), **combined via `Z(x)`** — not the product (§3.3); BBSE kept as baseline / anti-causal diagnostic only | BBSE conditioning diagnostic `κ(Ĉ_S)`; the `q̂_T` vs `Ĉ_S p̂_T` anti-causal sanity check; prevalence-correction error on `D_tar^lab` |
...
Under both shifts, the combined weight is the label ratio times a **corrected** covariate factor, **not** the product. The correction follows directly from the master importance-weighting identity of §1.5 (apply the law of total probability to `dP_T/dP_S`); it is a **known** identity, not a new theorem. It is the prior-shift posterior correction of Saerens, Latinne & Decaestecker (Neural Computation 2002) — whose per-`x` normalizer is exactly `Z(x)` — times the covariate ratio, and coincides with Tasche (2022, arXiv:2207.14514) **Theorem 4 / Corollary 4 at `ρ = 1`**, exact under the *invariant-density-ratio* assumption (Tasche 2014; JMLR 2017, arXiv:1701.05512). We restate it for the Hájek-weighted selective-risk estimator; our contribution here is the normalizer estimation and the reported diagnostics, not the identity.
...
**Why we adopt the factorizable-shift modeling premise.** Combining the two corrections presumes the two shift mechanisms can be treated as separable. Precisely, by **factorizable** we mean the *invariant-density-ratio* condition — the class-conditional density ratio `p_T(x∣y)/p_S(x∣y)` is **class-free on overlapping support** — which is exactly what makes the `Z`-combine exact (Tasche 2022, Thm 4/Cor 4 at `ρ=1`). This is **strictly stronger** than, and must not be confused with, the Factorizable-Joint-Shift (FJS) sense of He et al. (2022, arXiv:2203.02902) / Tasche, where factorizable means only `w(x,y)=U(x)·V(y)`: a shift can be FJS-factorizable yet violate our combine (a construction that factorizes as `U(x)·V(y)` to machine precision yet breaks the identity is hard-gated in the Gate-C tests). We adopt this invariant-density-ratio premise as a **modeling choice** for dense imaging shift, and do **not** adopt Sparse Joint Shift (Chen, Zaharia & Zou 2022) because its sparsity premise — only a few features shift — does not fit raw-image covariate shift, which is dense. The premise is **not identifiable from unlabeled target data alone** (which is why the labeled slice exists); we therefore fit a **simple scalar correction** on `D_tar^lab` and **report the residual gap empirically** (§3.4). There is no exactness claim and no certified combined weight.
...
CALIBRATION   (mutually DISJOINT folds: D_disc · D_bbse^src (labeled source) · D_tar (unlabeled target)
  ...
  1. DISCRIMINATOR.   Fit d(x) on D_disc (source-vs-target) by K-FOLD CROSS-FITTING:
                      each point scored out-of-fold, so weights are not fit on the
                      points they reweight.
                      ...
                      plus the weight-distribution summary (median/95/99/max; per-class &
                      per-site histograms) — n_eff alone hides tails (prereg §5.4).
  ...
  3. COMBINE (not multiply).  Ẑ(x) = Σ_{y'} ŵ_lab(y')·σ̃(f(x))_{y'}, with σ̃ a TEMPERATURE/Platt-recalibrated
                      ...
                      ENTANGLEMENT CHECK: on D_tar^lab also estimate a direct joint weight ŵ_joint(x,y)
                      and REPORT its divergence from the factorized ŵ (prereg §5.4); large divergence ⇒
                      factorization failing on this pair ⇒ downgrade the combined claim. ŵ_joint is a
                      slice-level diagnostic only, never deployed (not identifiable from unlabeled
                      target; high-variance at m_lab).
                      IMPLEMENTED as conformal.label_shift.residual_on_labeled_target (with
                      measure_slice_joint_weight): consumes the labeled slice, REPORTS the residual
                      risk gap (a) and the slice-measured-joint-vs-factorized divergence (b) — the
                      joint weight measured DIRECTLY from the labeled samples, never the oracle. It
                      MEASURES only (build_gates.md §6 hard gate); it identifies/certifies nothing.
  ...
  4. OOD.            Fit o(x) on (spectral-norm) features.  On O, set t_ood to the cutoff whose measured
                      ...
                      Report κ(Σ̂) and the effective rank of the Mahalanobis tied covariance Σ̂ as a
                      feature-collapse diagnostic (prereg §5.4).
...
- **Repository references.** `aangelopoulos/rcps`, `gostevehoward/confseq`, `aangelopoulos/ltt`, the label-shift code in `kundajelab/abstention` (BBSE/MLLS/BCTS — the `kundajelab/labelshiftexperiments` repo is the paper's experiment notebooks that import this package), and for the OOD detectors `mueller-mp/maha-norm` (Mahalanobis++, Müller et al. 2025 — deployed *with* its in-distribution-only L2-normalization as the primary; the plain Lee et al. 2018 tied-covariance score, normalization removed, is the canonical-baseline ablation), `deeplearning-wisc/knn-ood` (kNN ablation), `wetliu/energy_ood` (energy ablation), and `deeplearning-wisc/react` (ReAct wrap ablation).
...
- **Softmax plug-in bias.** The denominator `Ẑ(x)` uses a plug-in for `p_S(y∣x)`; because raw softmax is miscalibrated, we recalibrate `f` (temperature/Platt) before forming `Ẑ`. This is a measured bias-reduction step, reported, not a guarantee. Whether the residual survives depends on **well-specification** of the miscalibration relative to the BCTS `(T, b)` family: if the true distortion is **in-family** (a single global temperature + per-class bias), BCTS is consistent and the residual **vanishes with `n`** (in the synthetic gate-2 regime, `~0.015→0.001` as `n:5e3→5e5`); if it is **out-of-family** (e.g. a per-class temperature the single-`T` fit cannot invert), a **non-vanishing bias floor** remains (`~0.008` up to `~0.04` at the extreme tail) that recalibration reduces but does not remove, **amplified on rare / high-`ŵ_lab` classes** where a small denominator error scales a large weight. We therefore do **not** assume the bias reduced: its residual effect on the headline weighted risk is **quantified** by a pre-registered sensitivity that lives in the out-of-family regime (raw softmax vs BCTS-recalibrated vs a temperature band), and the `w_max` clip bias is **swept** the same way (prereg `docs/preregistration.md` §5.4 / §6.6).
...
Combining covariate and label weights by the **naïve product** `ŵ_cov(x)·ŵ_lab(y)` is *wrong* — it double-counts, because a prevalence change moves the covariate marginal `p(x)` and the discriminator picks that up. The correct combine divides by the per-`x` normalizer, which follows directly from the master importance-weighting identity of §1.5 (law of total probability on `dP_T/dP_S`); it is a **known** identity, not a new result — the Saerens, Latinne & Decaestecker (2002) prior-shift correction (its normalizer is `Z(x)`) times the covariate ratio, and exactly Tasche (2022, arXiv:2207.14514) **Theorem 4 / Corollary 4 at `ρ = 1`** under the invariant-density-ratio assumption (Tasche 2014):
```
Z(x) := Σ_{y'} w_lab(y') p_S(y'∣x) = E_{p_S(·∣x)}[ w_lab ],     Ẑ(x) = Σ_{y'} ŵ_lab(y') σ̃(f(x))_{y'}
w(x,y) = w_lab(y) · w_cov(x) / Z(x)
```
*Worked check:* under pure label shift `p_S(y)=(.5,.5) → p_T(y)=(.9,.1)`, the truth at an `x` where class 1 dominates is `w = 1.8`, while the naïve product gives `1.8·1.8 = 3.24`; dividing by `Z(x)` recovers `1.8` (the full derivation is in §3.3). *Risk-space bias direction.* The product's per-`x` inflation by `Z(x)` is exact as a factor and inflates the aggregate weight (`E_S[w_lab·Z] > 1`), but the sign of its bias on the self-normalized (Hájek) accepted risk is `sign(cov(Z, ℓ))` on the accepted set: where confident (low-`Z`, easy majority) points carry the low loss, `cov(Z, ℓ) < 0` and the product arm certifies a risk that is *too low* — anti-conservative. On the committed K=2 accepted region the product arm reports Hájek risk `0.0515` against the true `R_T ≈ 0.1026` (the `Ẑ`-divide recovers `0.1024`); the danger of the naïve product is thus not only variance but an under-stated risk. We use the **temperature/Platt-recalibrated** softmax `σ̃` inside `Ẑ` because the raw softmax denominator is a non-vanishing plug-in bias — this is a genuine estimation fix, not a certificate. The bias is **not assumed reduced**: it is **non-vanishing** even after recalibration and is **amplified on rare / high-`ŵ_lab` classes**, so its residual effect on the headline weighted risk is **quantified** by a pre-registered sensitivity (raw vs BCTS vs a temperature band), and the `w_max` clip bias is **swept** likewise (prereg `docs/preregistration.md` §5.4 / §6.6).

**Modeling choice, adopted not proven.** We adopt a factorizable shift premise — the *invariant-density-ratio* condition `p_T(x∣y)/p_S(x∣y)` class-free on overlapping support (Tasche 2022 Thm 4/Cor 4 at `ρ=1`), **strictly stronger** than the FJS `w(x,y)=U(x)·V(y)` sense of He et al. (2022) — as a modeling choice; it is not identifiable from unlabeled target data, so we fit a scalar correction on `D_tar^lab` and report the residual. Sparse Joint Shift is not adopted because raw-image shift is dense. The premise is engaged empirically, not merely asserted, in two pre-registered ways: (i) a **real-data combine ablation** sweeps no-reweight / covariate-only / label-only / naïve-product / `Ẑ`-divide, so the double-count correction must *earn* its place against the naïve product on held-out target, not only in the §3.3 worked example (prereg §4A); and (ii) a **factorization-entanglement diagnostic** reports the divergence on `D_tar^lab` between the factorized weight and a directly-estimated joint weight `ŵ_joint` (prereg §5.4) — making the independence premise **falsifiable per hospital pair**: where prevalence and acquisition are entangled (`p(x∣y)` moving — the correlated-acquisition regime a clinical reviewer will rightly raise), the divergence is large and the combined claim is **downgraded** for that pair rather than asserted. The joint weight is used only as this slice-level check, since it too needs labeled target to identify and is high-variance at `m_lab ≈ 50–200`.
