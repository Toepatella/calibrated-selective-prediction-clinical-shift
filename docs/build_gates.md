...
- `tests/test_gate_c_label_shift.py` — **implemented; 16 tests (7 hard gates + 9 diagnostics), all passing.**
...
- **Recalibrator + module + generator written.** `conformal/label_shift.py` (with `fit_bcts`/`recalibrate_softmax` feeding `Ẑ`), `tests/_synth.py::LabelShiftMixture`, and `tests/test_gate_c_label_shift.py` all exist and pass (16/16); the earlier "empty test / no fixture" gaps are closed.
...
- G-C (`conformal/label_shift.py` MLLS+BCTS/BBSE + `Ẑ`-combine) — **implemented (16/16); G-D stacks the OOD screen on the combined-weight pipeline.**
...
- Reuse of G-A `conformal/rcps.py`/`selective.py`, G-B `weights.py`, and G-C `label_shift.py` (all implemented) — the `Ẑ`-combined weight G-D routes on now exists.
...
- G-B (`weights.py`) and G-C (`label_shift.py`) are both implemented, so the `Ẑ`-combined weight G-D routes on exists — **G-D is now unblocked** (it is the next rung on the ladder).
